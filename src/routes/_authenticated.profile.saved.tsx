      ...
      <div className="px-5 pt-2">
        {items.length === 0 ? (
          ...
          <ul className="space-y-3">
            {items.map((d) => (
              <li key={d.id} className="relative overflow-hidden rounded-3xl bg-card shadow-soft">
                <Link to="/destinations/$id" params={{ id: d.id }} className="flex gap-3">
                  ...
                  <div className="flex flex-1 flex-col justify-center py-2 pr-14">
                ...
                <button
                  onClick={() => remove(d.id)}
                  className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-background/95 text-destructive shadow-soft"
